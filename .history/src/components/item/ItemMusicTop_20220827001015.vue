...
export default {
  ...
  methods:{
    ...
    // CommentShow:async function(){
    //   let res = await getCommentList(this.playList.id)
    //   console.log(res)
    // }
  },
  mounted(){
  ...
  setup(props){
    ...
    if((props.id="")){
      props.id.creator=sessionStorage.getItem('itemDetail')
    }
     // 对播放量的处理
    ...
    async function CommentShow(){
      let res = await getCommentList(this.id)
      console.log(this.id)
      console.log(res)
    }
    return{changeCount,CommentShow}
  },
  props:['playList','id']
}
</script>
