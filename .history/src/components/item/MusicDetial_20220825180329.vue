<template>
  <div class="musicDetail">
    ...
    <div class="lyricList" ref="lyricList" v-show="isLyricShow" @click="isLyricShow=false">
      <!-- 在出现该歌词到下一句歌词出现之间 -->
...
export default {
  ...
  computed:{
    ...mapState(['lyricList',"currentTime",'duration','songList','songListIndex']),
    // 分割歌词
    lyric:function(){
      let arr;
      if(this.lyricList.lyric)
      {
        ...
        arr.forEach((item,index) =>{
          // index===arr.length-1表示如果是最后一句歌词就将pre（下一句歌词的时间）属性修改为100000
          // 为了让最后一句歌词高亮于是添加了isNaN(arr[index+1].time 如果下一句歌词是时间是NaN 
          // 就把倒数第二句的pre属性修改 实现最后一句高亮
          if(index===arr.length-1 || isNaN(arr[index+1].time)){
            // 增加属性，表示下一句歌词的时间
            item.pre = 100000000000000
          }else{
            // 获取下一句歌词的时间
  ...
  watch:{
    ...
    currentTime:function(newValue){
      ...
      if(p){
        if(p.offsetTop>300){
          this.$refs.lyricList.scrollTop = p.offsetTop-300
        }
      }
      // console.log([this.$refs.lyricList]);
      // 如果当当前时间到了歌曲的总时长 就播放下一首个
      if(newValue === this.duration){
        if(this.songListIndex === this.songList.length-1){
          this.updateSongListIndex(0)//默认列表不会自动播放
          this.play()
        }else{
          this.updateSongListIndex(this.songListIndex+1)
        }
      }
    }
  }
