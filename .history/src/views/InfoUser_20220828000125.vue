<template>
  <div class="UserTop">
    <img :src="user.data.profile.backgroundUrl" alt="" class="backPic">
     <svg class="icon" aria-hidden="true" @click="$router.push('/')">
          <use xlink:href="#icon-zuojiantou"></use>
    </svg>
    <!-- <img src="../assets/网易云音乐-01.svg" alt="" class="backPic"> -->
    <div class="User">
        <img :src="user.data.profile.avatarUrl" alt="" class="nickPic">
        <p class="nickname">{{user.data.profile.nickname}}</p>
...
import {getLoginOut,getLoginStatus} from '../request/api/home.js'
export default {
  computed:{
    ...mapState(['user'])
  },
  mounted(){
    this.usermgs= JSON.parse(localStorage.getItem('usermgs')) ? JSON.parse(localStorage.getItem('usermgs')) : [],
  ...
  methods:{
    ...
    User:async function(){
      ...
      if(res.data.data.code === 200){
         // 登录成功立即获取用户详情 
        let result = await getLoginUser(id)
      }
    }
  } 
}
